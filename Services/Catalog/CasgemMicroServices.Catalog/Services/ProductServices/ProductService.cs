...
namespace CasgemMicroServices.Catalog.Services.ProductServices
{
    public class ProductService : IProductService
    {
        ...
        public async Task<Response<CreateProductDto>> GetProductAsync(CreateProductDto createProduct)
        {
            var value = _mapper.Map<Product>(createProduct);
            await _productCollection.InsertOneAsync(value);
            return Response<CreateProductDto>.Success(_mapper.Map<CreateProductDto>(value), 204);
        ...
        public async Task<Response<UpdateProductDto>> UpdateProductAsync(UpdateProductDto updateProduct)
        {
            var value = _mapper.Map<Product>(updateProduct);
            var result = await _productCollection.FindOneAndReplaceAsync(x => x.ProductID == updateProduct.ProductID, value);
            if (result == null)
            {
            ...
            {
                return Response<UpdateProductDto>.Success(200);
            }
